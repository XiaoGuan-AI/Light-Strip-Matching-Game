...
void Display::Update() {
    ...
    // Monitor stack usage to detect potential overflow early
    #if CONFIG_FREERTOS_GENERATE_RUN_TIME_STATS
    static uint32_t update_count = 0;
    if (++update_count % 60 == 0) {  // Log every 60 seconds
        TaskHandle_t current_task = xTaskGetCurrentTaskHandle();
        UBaseType_t high_water_mark = uxTaskGetStackHighWaterMark(current_task);
        if (high_water_mark < 512) {
            ESP_LOGW(TAG, "Display update task stack low: %u bytes remaining", high_water_mark * sizeof(StackType_t));
        }
    }
    #endif
...
void Display::SetTheme(const std::string& theme_name) {
    ...
    ESP_LOGI(TAG, "Theme set to: %s", theme_name.c_str());
}
