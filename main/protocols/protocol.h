...
public:
    ...
    // 模式切换：用于与后端同步显式的对话/聆听模式
    void SendModeCommand(const std::string& action);
    // 发送打印结果
    void SendPrintResult(const std::string& task_id, const std::string& status, const std::string& error = "");
    // 发送语音打印指令 (STT结果)
    void SendVoicePrintCommand(const std::string& text, bool is_final = true);
    // 发送语音会话控制
    void SendVoiceSessionControl(const std::string& control_type, const std::string& wake_word = "");
