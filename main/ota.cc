...
bool Ota::CheckVersion() {
    ...
    // 发送打印机型号，用于 OTA 服务器区分 EM20L/EM30L 固件
#if defined(CONFIG_PRINTER_MODEL_EM20L)
    http->SetHeader("Printer-Model", "EM20L");
#elif defined(CONFIG_PRINTER_MODEL_EM30L)
    http->SetHeader("Printer-Model", "EM30L");
#endif
    ...
    // 打印服务器返回的原始数据
    ESP_LOGI(TAG, "Server response data: %s", data.c_str());
    // Enable display only after OTA info received (avoids early panel flicker)
    Board::GetInstance().EnsureDisplayEnabled();
    ...
    // 检查 activation 不是 null 且不是 cJSON_NULL 类型
    if (activation != NULL && !cJSON_IsNull(activation)) {
    ...
    if (server_time != NULL) {
        ...
        if (timestamp != NULL) {
            ...
            // 立刻刷新一次时间显示（不必等待 SNTP 回调），放到应用主循环执行
            Application::GetInstance().Schedule([](){
                auto d = Board::GetInstance().GetDisplay();
                if (d) d->UpdateTimeOnly();
            });
        }
    }
    ...
    if (firmware == NULL) {
        ...
        // 打印固件对象
        char *firmware_str = cJSON_Print(firmware);
        if (firmware_str) {
            ESP_LOGI(TAG, "Firmware object: %s", firmware_str);
            free(firmware_str);
        }
    }
