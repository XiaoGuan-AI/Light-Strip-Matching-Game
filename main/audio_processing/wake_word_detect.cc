...
void WakeWordDetect::Feed(const std::vector<int16_t>& data) {
    if (afe_iface_ == nullptr || afe_data_ == nullptr || !IsDetectionRunning()) {
        return;
    }
...
size_t WakeWordDetect::GetFeedSize() {
    if (afe_iface_ == nullptr || afe_data_ == nullptr || codec_ == nullptr || !IsDetectionRunning()) {
        return 512; // 安全默认值
    }
